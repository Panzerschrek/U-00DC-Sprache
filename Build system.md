...
A custom target is just a target with a custom build rule, specified inside the build file via a custom functor.
...
Such build target should be build for host triple in order to be used as a build command.
...
There should be a way for language server to interact with the build system - to obtain list of project's files and import directories.
