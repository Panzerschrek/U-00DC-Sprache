...
mkdir build-travis &&\
cd build-travis &&\
cmake ../source/ -G Ninja -DCMAKE_BUILD_TYPE=Release -DLLVM_LIB_DIR=../clang+llvm-9.0.0-x86_64-linux-gnu-ubuntu-16.04/lib/ -DU_BUILD_COMPILER2=YES -DSPHINX_WARNINGS_AS_ERRORS=No &&\
\
cmake --build . &&\
\
# Run ustlib tests
