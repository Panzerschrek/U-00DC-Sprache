...
def ReturningUnallowedReference_ForAsyncReturn_Test9():
	c_program_text= """
		struct S{ i32& x; }
...
def ReturningUnallowedReference_ForReferencePassedThroughAsyncFunction_Test1():
	c_program_text= """
		...
		fn Bar( i32& x ) : i32& @( bar_return_references )
		{
			var S s{ .x= x };
			auto mut gen= Foo( s );
			if_coro_advance( s_copy : gen )
			{
				return s_copy.x; // This returned reference is linked to "s.x", which is linked to reference param "x". Returning a reference to it isn't allowed.
			}
			halt;
		}
		var [ [ char8, 2 ], 0 ] bar_return_references[ ];
...
def ReturningUnallowedReference_ForReferencePassedThroughAsyncFunction_Test2():
	c_program_text= """
		struct S{ i32& x; }
		var tup[ [ [ char8, 2 ], 1 ] ] return_inner_references[ [ "0a" ] ];
		fn async Foo( S s ) : S @(return_inner_references);
...
def ReturningUnallowedReference_ForReferencePassedThroughAsyncFunction_Test3():
	c_program_text= """
		struct S{ i32& x; }
		var [ [ char8, 2 ], 1 ] return_references[ "0a" ];
		fn async Foo( S s ) : i32& @(return_references);
...
def NonDefaultCallingConventionForCoroutine_ForAsyncFunction_Test3():
	c_program_text= """
		fn async Foo() call_conv("C") : i32 { return 0; } // "C" is not default calling convention.
	"""
	errors_list= ConvertErrors( tests_lib.build_program_with_errors( c_program_text ) )
	assert( len(errors_list) > 0 )
	assert( HasError( errors_list, "NonDefaultCallingConventionForCoroutine", 2 ) )
...
def SecondOrderInnerReferenceForCoroutineReferenceProtectionViolation_Test1():
	# This test demonstrates violation of reference protection caused by second order inner references.
	c_program_text= """
		struct S
		{
		...
		fn async Func( T t ) : i32
		{
			auto &mut x= t.s.x; // Hold a mutable reference to "s.x" across "yield" points. According to langauge rules noone can change underlying value while this reference exists.
			auto x_before= x;
			yield;
			halt if( x != x_before ); // In normal code a value pointed by a mutable reference can't be changed by someone else. But "yield" and second order inner references for coroutines allow this.
			++x;
