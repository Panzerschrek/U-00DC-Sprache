	...
	const TemplateSignatureParam::Tuple& tuple_type_param )
{
	...
	elements_llvm_types.reserve( tuple_type_param.element_types.size() );
	...
	const TemplateSignatureParam::Function& function_param )
{
	...
	for( const TemplateSignatureParam::Function::Param& param : function_param.params )
	{
...
{
	const llvm::StringRef completion_name_ref= completion_name;
	const llvm::StringRef value_name_ref= value_name;
	...
	{
		if( value_name_ref.startswith( Keyword(Keywords::static_assert_) ) || // static_assert name may exist inside namespace, but we should ignore it.
		...
		if( const auto functions_set= value.GetFunctionsSet() )
		{
			...
			if( functions_set->functions.empty() &&
				functions_set->template_functions.empty() &&
				functions_set->syntax_elements.empty() &&
