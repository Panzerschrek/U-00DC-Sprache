	...
	if( function_context.lambda_preprocessing_context != nullptr )
	{
		if( const auto variable= result.value->value.GetVariable() )
		{
			if( function_context.lambda_preprocessing_context->external_variables.count( variable ) > 0 )
				return LambdaPreprocessingAccessExternalVariable( names_scope, function_context, variable, name_lookup.name, name_lookup.src_loc );
		}
	}
...
Value CodeBuilder::ResolveValueImpl( NamesScope& names_scope, FunctionContext& function_context, const Synt::TemplateParameterization& template_parameterization )
{
	...
	else if( const OverloadedFunctionsSetPtr functions_set= base.GetFunctionsSet() )
	{
		...
		{
			REPORT_ERROR( ValueIsNotTemplate, names_scope.GetErrors(), template_parameterization.src_loc );
			return ErrorValue();
		}
		...
		return parameterized_functions;
	}
	else if( const auto this_overloaded_methods_set= base.GetThisOverloadedMethodsSet() )
	{
		...
		return ThisOverloadedMethodsSet{ this_overloaded_methods_set->this_, parameterized_functions };
	}
	else
	{
		REPORT_ERROR( ValueIsNotTemplate, names_scope.GetErrors(), template_parameterization.src_loc );
