...
* Класс аренного аллокатора (arena_allocator.u).
* Ассерты (assert.u).
...
* Контейнер массива, выделяемого в арене (arena_allocated_array.u).
* Контейнер-обёртка (arena_allocated_box.u), с хранилищем, выделяемым в арене и его зануляемая версия (arena_allocated_box_nullable.u).
