...
Класс может иметь не более одного не-интерфейсного предка и любое количество интерфейсных предков.
Неинтерфейсный предковый класс считается базой класса. Обратиться к нему можно с использованием ключевого слова ``base``.
``base`` - это ``this``, но преобразованый в тип базового класса.
...
* ``final`` класс, это такой полиморфный класс, от котогоро уже нельзя унаследоваться.
* ``abstract`` класс, это такой полиморфный класс, у которого могут быть нереализованные виртуальные методы. У полиморфных классов, не помеченных как ``abstract`` или ``interface`` все виртуалньые методы должны иметь реализацию.
...
* ``virtual pure`` - метод впервые объявлется как виртуальный. Реализация данного метода запрещена.
