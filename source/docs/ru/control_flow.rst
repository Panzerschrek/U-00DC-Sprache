...
В Ü есть цикл ``loop``, который является самым простым циклом.
...
Важной особенностью цикла ``loop`` является тот факт, что если тело цикла не содержит ``break`` операторов (относящихся к этому циклу), код после оператора ``loop`` считается недостижимым.
