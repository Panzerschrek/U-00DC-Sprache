...
Главное правило контроля ссылок звучит так: в каждой точке потока управления переменная или ссылка должны иметь или только одну производную изменяемую ссылку или от нуля до бесконечности производных неизменяемых ссылок.
...
Но бывают функции, которые в действительности возвращают ссылки не на все ссылочные аргументы.
Чтобы избежать излишнего учёта производных ссылок в таких случаях, такие функции надо аннотировать специальным образом.
...
.. code-block:: u_spr
   ...
   fn Bar()
   {
       ...
       Foo( s, y ); // После этого вызова "s.r" является производной ссылкой ещё и от "y"
   }
...
Данное выражение позволяет сопоставить внутренние ссылки типа поля с ссылочными тегами структуры.
...
.. code-block:: u_spr
   ...
   struct S
   {
       i32& @('a') x; // Ссылка указывает на тег "a" (#0).
       i32& @('b') y; // Ссылка указывает на тег "b" (#1).
   }
   ...
   // Этому тегу соответствуют ссылочные поля "i0" и "i1".
...
В примерах ниже показано, как правило контроля ссылок осуществляется на практике.
...
.. code-block:: u_spr
   ...
   {
       var f64 mut x= 0.0;
       MutateArgs( x, x ); // Будет порождена ошибка. Для вызова функции одновременно создаются две производные от переменной "x" изменяемые ссылки.
   }
