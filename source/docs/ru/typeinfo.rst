...
* ``tup[] functions_list`` - кортеж, каждый элемент которого описывает функцию структуры/класса
* ``tup[] function_templates_list`` - кортеж, каждый элемент которого описывает шаблон функции структуры/класса
* ``tup[] parents_list`` - кортеж, каждый элемент которого описывает родительский класс

Описание каждого поля, вложенного типа, функции, шаблона функции содержит:
...
Описание каждого шаблона функции содержит:

* ``size_type param_count`` - количество параметров шаблонной функции
* ``bool is_this_call`` - первый аргумент функции - ``this``
...
* ``call_conv`` - массив элементов ``char8``, содержащий имя соглашения о вызове
