...
mkdir build_dir &&\
cd build_dir &&\
cmake ../source/ -G Ninja -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_C_FLAGS=-m32 -DCMAKE_CXX_FLAGS=-m32 -DCMAKE_EXE_LINKER_FLAGS=-m32 -DCMAKE_SHARED_LINKER_FLAGS=-m32 -DCMAKE_BUILD_TYPE=Release -DCMAKE_MODULE_PATH=$PWD/../cmake/Modules/ -DLLVM_SRC_DIR=$PWD/../llvm-17.0.6.src/ -DLLVM_EXTERNAL_CLANG_SOURCE_DIR=$PWD/../clang-17.0.6.src/ -DLLVM_EXTERNAL_LLD_SOURCE_DIR=$PWD/../lld-17.0.6.src/ -DLLVM_TOOL_CLANG_BUILD=ON -DLLVM_TOOL_LLD_BUILD=ON -DU_BUILD_COMPILER2=YES -DU_BUILD_COMPILER3=YES -DU_BUILD_INTERPRETER=OFF -DU_BUILD_PY_TESTS=OFF -DU_BUILD_BUILD_SYSTEM_TESTS=OFF -DLLVM_TARGETS_TO_BUILD=X86 -DLLVM_NATIVE_ARCH="X86" -DLLVM_BUILD_BENCHMARKS=OFF -DLLVM_INCLUDE_BENCHMARKS=OFF -DLLVM_BUILD_DOCS=OFF -DLLVM_BUILD_EXAMPLES=OFF -DLLVM_BUILD_TESTS=OFF -DLLVM_INCLUDE_TESTS=OFF -DLLVM_DEFAULT_TARGET_TRIPLE=i686-unknown-linux-gnu -DLLVM_HOST_TRIPLE=i686-unknown-linux-gnu -DLLVM_ENABLE_TERMINFO=OFF -DLLVM_ENABLE_LIBXML2=OFF -DLLVM_ENABLE_ZLIB=OFF -DLLVM_ENABLE_ZSTD=OFF -DLLVM_DISABLE_ASSEMBLY_FILES=ON &&\
\
# Run build
cmake --build . &&\
\
