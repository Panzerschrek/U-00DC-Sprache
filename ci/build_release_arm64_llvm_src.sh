...
mkdir build_dir &&\
cd build_dir &&\
cmake ../source/ -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_MODULE_PATH=$PWD/../cmake/Modules/ -DLLVM_SRC_DIR=$PWD/../llvm-17.0.6.src/ -DLLVM_EXTERNAL_LLD_SOURCE_DIR=$PWD/../lld-17.0.6.src/ -DLLVM_TOOL_CLANG_BUILD=OFF -DLLVM_TOOL_LLD_BUILD=ON -DU_BUILD_COMPILER2=NO -DU_BUILD_COMPILER3=NO -DLLVM_TARGETS_TO_BUILD=AArch64 -DLLVM_NATIVE_ARCH="AArch64" -DLLVM_BUILD_BENCHMARKS=OFF -DLLVM_INCLUDE_BENCHMARKS=OFF -DLLVM_BUILD_DOCS=OFF -DLLVM_BUILD_EXAMPLES=OFF -DLLVM_BUILD_TESTS=OFF -DLLVM_INCLUDE_TESTS=OFF &&\
\
# Run build
cmake --build . -- -j 1 -k 1024 &&\
\
# Run ustlib tests
cd .. &&\
