...
xz -d clang+llvm-10.0.1-x86_64-linux-gnu-ubuntu-16.04.tar.xz &&\
tar -xf clang+llvm-10.0.1-x86_64-linux-gnu-ubuntu-16.04.tar &&\
\
# Configure build
mkdir build-travis &&\
cd build-travis &&\
cmake ../source/ -G Ninja -DCMAKE_BUILD_TYPE=Release -DLLVM_LIB_DIR=../clang+llvm-10.0.1-x86_64-linux-gnu-ubuntu-16.04/lib/ -DU_BUILD_COMPILER2=YES -DU_BUILD_COMPILER3=YES -DSPHINX_WARNINGS_AS_ERRORS=No &&\
\
cmake --build . &&\
\
# Run ustlib tests
